...
func (st *insertStats) report(chain []*types.Block, index int, dirty common.StorageSize, proctime time.Duration, wtime time.Duration) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*/
		context := []interface{}{
			"number", end.Number(), "txs", txs, "mgas", float64(st.usedGas) / 1000000, "hash", end.Hash(),
			"elapsed", common.PrettyDuration(elapsed), "mgasps", float64(st.usedGas) * 1000 / float64(elapsed),
			"proctime", common.PrettyDuration(proctime), "wtime", common.PrettyDuration(wtime),
			"blocks", st.processed,
		}

...
var (
	...
	TargetGasLimitCalculationFlag = cli.BoolFlag{
		Name:  "targetgaslimitcalculation",
		Usage: "Tell whether enable or disable target gas limit calculation",
	}
	TxGasLimitFlag = cli.Uint64Flag{
		Name:  "txgaslimit",
		Usage: "Tx gas limit sets the gas floor for the tx and call to execute",
		Value: params.TxGasLimit,
	}
	TxCountLimitFlag = cli.Uint64Flag{
		Name:  "txcountlimit",
		Usage: "Tx count limit sets the upper bound for the total number of txs included in block",
		Value: params.TxCountLimit,
	}
	TxCommitNanosecondsFlag = cli.Uint64Flag{
		Name:  "txcommitnanoseconds",
		Usage: "Maximum time allowed to execute transactions",
		Value: params.TxCommitNanoseconds,
	}
	EvmTimeoutFlag = cli.Uint64Flag{
		Name:  "evmtimeoutnanoseconds",
		Usage: "EVM timeout (nanoseconds)",
		Value: params.EvmTimeoutNanoseconds,
	}
	CallTimeoutFlag = cli.Uint64Flag{
		Name:  "calltimeoutnanoseconds",
		Usage: "Call timeout (nanoseconds)",
		Value: params.CallTimeoutNanoseconds,
	}
	DisableGhostStateAttackProtectionFlag = cli.BoolFlag{
		Name:  "disableghoststateattackprotection",
		Usage: "Tell whether disable Ghost-State-Attack protection",
	}
	SignerFlag = cli.StringFlag{
		Name:  "signer",
		Usage: "Public address for block mint",
		Value: "0",
	}
	// Account settings
	...
	// RPC settings
	RPCDebugLoggingFlag = cli.BoolFlag{
		Name:  "rpclog",
		Usage: "Enable RPC debug logging",
	}
...
// setSigner creates the signer address string from the set
// command line flags, returning empty if the signer address is disabled.
func setSigner(ctx *cli.Context, ks *keystore.KeyStore, cfg *ethconfig.Config) {
	if ctx.GlobalIsSet(SignerFlag.Name) {
		account, err := MakeAddress(ks, ctx.GlobalString(SignerFlag.Name))
		if err != nil {
			Fatalf("Option %q: %v", SignerFlag.Name, err)
		}
		cfg.Signer = account.Address
	}
}
...
func SetNodeConfig(ctx *cli.Context, cfg *node.Config) {
	...
	if ctx.GlobalIsSet(RPCDebugLoggingFlag.Name) {
		cfg.RPCDebugLogging = true
	}
}
...
func setMiner(ctx *cli.Context, cfg *miner.Config) {
	...
	if ctx.GlobalIsSet(TargetGasLimitFlag.Name) {
		cfg.TargetGasLimit = ctx.GlobalUint64(TargetGasLimitFlag.Name)
		params.TargetGasLimit = ctx.GlobalUint64(TargetGasLimitFlag.Name)
	}
	if ctx.GlobalIsSet(TargetGasLimitCalculationFlag.Name) {
		cfg.TargetGasLimitCalculation = true
		params.TargetGasLimitCalculation = true
	}
	if ctx.GlobalIsSet(TxGasLimitFlag.Name) {
		cfg.TxGasLimit = ctx.GlobalUint64(TxGasLimitFlag.Name)
		params.TxGasLimit = ctx.GlobalUint64(TxGasLimitFlag.Name)
	}
	if ctx.GlobalIsSet(TxCountLimitFlag.Name) {
		cfg.TxCountLimit = ctx.GlobalUint64(TxCountLimitFlag.Name)
		params.TxCountLimit = ctx.GlobalUint64(TxCountLimitFlag.Name)
	}
	if ctx.GlobalIsSet(TxCommitNanosecondsFlag.Name) {
		cfg.TxCommitNanoseconds = ctx.GlobalUint64(TxCommitNanosecondsFlag.Name)
		params.TxCommitNanoseconds = ctx.GlobalUint64(TxCommitNanosecondsFlag.Name)
	}
	if ctx.GlobalIsSet(EvmTimeoutFlag.Name) {
		cfg.EvmTimeoutNanoseconds = ctx.GlobalUint64(EvmTimeoutFlag.Name)
		params.EvmTimeoutNanoseconds = ctx.GlobalUint64(EvmTimeoutFlag.Name)
	}
	if ctx.GlobalIsSet(CallTimeoutFlag.Name) {
		cfg.CallTimeoutNanoseconds = ctx.GlobalUint64(CallTimeoutFlag.Name)
		params.CallTimeoutNanoseconds = ctx.GlobalUint64(CallTimeoutFlag.Name)
	}
	if ctx.GlobalIsSet(DisableGhostStateAttackProtectionFlag.Name) {
		cfg.DisableGhostStateAttackProtection = true
		params.DisableGhostStateAttackProtection = true
	}
}

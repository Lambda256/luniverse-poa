...
func (eth *Ethereum) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (*common.ConsensusConfig, core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	if parent == nil {
		return nil, nil, vm.BlockContext{}, nil, fmt.Errorf("parent %#x not found", block.ParentHash())
	}
	...
	if eth.blockchain.Config() != nil && eth.blockchain.Config().Clique != nil && eth.blockchain.Config().Clique.HasConsensusConfiguration() {
		consensusConfig = core.RetrieveConsensusConfigurations(eth.blockchain, parent.Header(), statedb, eth.blockchain.Config())
	}
	...
	for idx, tx := range block.Transactions() {
		...
		msg, _ := tx.AsMessage(eth.blockchain.Config(), signer, block.BaseFee())

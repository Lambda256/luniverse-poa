...
// GetTxGasLimit TxGasLimit returns tx gas limit.
func (s *PublicEthereumAPI) GetTxGasLimit(ctx context.Context) uint64 {
	return s.b.GetPoolTxGasLimit()
}
...
// GetEvmTimeoutNanoseconds returns timeout value of EVM execution in nanoseconds
func (s *PublicBlockChainAPI) GetEvmTimeoutNanoseconds() uint64 {
	return s.b.GetEvmTimeoutNanoseconds()
}
...
func (s *PublicBlockChainAPI) GetBalance(ctx context.Context, address common.Address, blockNrOrHash rpc.BlockNumberOrHash) (*hexutil.Big, error) {
	...
	if s.b.ChainConfig() != nil && s.b.ChainConfig().GasPoint != nil && s.b.ChainConfig().IsGasPointBlock(s.b.CurrentBlock().Number()) {
		...
		//evm := vm.NewEVM(context, state, s.b.ChainConfig(), vm.Config{})
		...
		//}
		//// decode output data
		//log.Debug("getUserGasPoint()", "addr", dcAddress, "response", common.ToHex(res))
		//// decode output data
		//gasPoint.SetBytes(res[:32])
	}
	return (*hexutil.Big)(new(big.Int).Add(b, gasPoint)), state.Error()
}

// GetNativeBalance returns the amount of wei for the given address in the state of the
// given block number. The rpc.LatestBlockNumber and rpc.PendingBlockNumber meta
// block numbers are also allowed.
func (s *PublicBlockChainAPI) GetNativeBalance(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (*big.Int, error) {
...
// GetGasPoint returns the amount of wei for the given address in the state of the
// given block number. The rpc.LatestBlockNumber and rpc.PendingBlockNumber meta
// block numbers are also allowed.
func (s *PublicBlockChainAPI) GetGasPoint(ctx context.Context, address common.Address, blockNr rpc.BlockNumber) (*big.Int, error) {
	state, _, err := s.b.StateAndHeaderByNumber(ctx, blockNr)
	if state == nil || err != nil {
		return nil, err
	}
	...
	return gasPoint, state.Error()
}

// GetBlockHeaderByNumber returns the requested block header. When blockNr is -1 the chain head is returned.
func (s *PublicBlockChainAPI) GetBlockHeaderByNumber(ctx context.Context, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
	block, err := s.b.BlockByNumber(ctx, blockNr)
	if block != nil {
		response, err := s.rpcMarshalBlock(ctx, block, false, false)
...
// GetFullBlockByNumber returns the requested block with full information. When blockNr is -1 the chain head is returned.
func (s *PublicBlockChainAPI) GetFullBlockByNumber(ctx context.Context, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
	block, err := s.b.BlockByNumber(ctx, blockNr)
	if block != nil {
		response, err := RPCMarshalBlock(block, true, true, s.b.ChainConfig())
		if err == nil {
			if blockNr == rpc.PendingBlockNumber {
				...
				// add receipts to result
				blockHash := block.Hash()
				blockNumber := block.NumberU64()
				txs := block.Body().Transactions

				receipts := rawdb.ReadReceipts(s.b.ChainDb(), blockHash, blockNumber, s.b.ChainConfig())
				receiptOutputs := make([]map[string]interface{}, 0, len(receipts))
				for index, receipt := range receipts {
					...
					from, _ := types.Sender(signer, tx) // cached value will be used (i.e., s.rpcOutputBlock(...) resolved sender and cached it)
					...
					receiptOutputs = append(receiptOutputs, fields)
				}
				response["receipts"] = receiptOutputs
			}
		}
...
func DoCall(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, overrides *StateOverride, timeout time.Duration, globalGasCap uint64) (*core.ExecutionResult, error) {
	if timeout <= time.Duration(0) {
		timeout = time.Duration(params.CallTimeoutNanoseconds)
	}
	startTime := time.Now()
	defer func(start time.Time) { log.Debug("Executing EVM call finished", "runtime", time.Since(start)) }(startTime)
	...
	if b.ChainConfig() != nil && b.ChainConfig().Clique != nil && b.ChainConfig().Clique.HasConsensusConfiguration() {
	...
	if poolGasPrice == nil {
		return nil, core.ErrUnexpectedLesNode /* currently implementation of LES node does not support `poolGasPrice` information */
	}
	...
	if gas == 0 {
		...
		gas = txGasLimit
	} else if txGasLimit < gas {
		return nil, core.ErrTxGasLimit
	}
	if gasPrice.Sign() == 0 {
	...
	defer func(start time.Time) {
		...
		cancel()
		log.Debug("Call() - evm execution done", "timeout", timeout, "elapsed", time.Since(start))
	}(time.Now())

	// Get a new instance of the EVM.
	msg, err := args.ToMessage(b.ChainConfig(), txGasLimit /*globalGasCap*/, nil /*header.BaseFee*/)
	if err != nil {
		return nil, err
	}
	...
	// Execute the message.
	gp := new(core.GasPool).AddGas(gas /*math.MaxUint64*/)
	result, err := core.ApplyMessage(consensusConfig, evm, msg, gp)
...
func DoEstimateGas(ctx context.Context, b Backend, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, gasCap uint64) (hexutil.Uint64, error) {
	...
	executable := func(gas uint64) (bool, *core.ExecutionResult, error) {
		...
		if err != nil {
			log.Debug("failed binary search in estimateGas()", "err", err, "args.Gas", args.Gas)
...
func AccessList(ctx context.Context, b Backend, blockNrOrHash rpc.BlockNumberOrHash, args TransactionArgs) (acl types.AccessList, gasUsed uint64, vmErr error, err error) {
	...
	for {
		...
		var consensusConfig *common.ConsensusConfig = nil
		if b.ChainConfig() != nil && b.ChainConfig().Clique != nil && b.ChainConfig().Clique.HasConsensusConfiguration() {
			consensusConfig = core.RetrieveConsensusConfigurations(b.BlockChain(), header, statedb, b.ChainConfig())
		}
		txGasLimit := params.TxGasLimit
		if consensusConfig != nil && consensusConfig.GasLimit != nil {
			txGasLimit = consensusConfig.GasLimit.Uint64()
		...
		msg, err := args.ToMessage(b.ChainConfig(), txGasLimit /*b.RPCGasCap()*/, nil /*header.BaseFee*/)
		if err != nil {
			return nil, 0, nil, err
		}
		...
		vmenv, _, err := b.GetNonTxEVM(ctx, msg, statedb, header, &config, (*big.Int)(args.Value))
		if err != nil {
			return nil, 0, nil, err
		}
		res, err := core.ApplyMessage(consensusConfig, vmenv, msg, new(core.GasPool).AddGas(msg.Gas()))
		if err != nil {

	...
	for i := 0; i < 3; i++ {
		...
		if bc != nil {
			...
			if err == nil {
				...
				msg := callmsg{types.NewMessage(config, from.Address(), &testContractAddr, 0, new(big.Int), 100000, big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee), new(big.Int), data, nil, true)}
				...
				result, _ := core.ApplyMessage(nil, vmenv, msg, gp)
			...
			msg := callmsg{types.NewMessage(config, bankAddr, &testContractAddr, 0, new(big.Int), 100000, big.NewInt(params.InitialBaseFee), big.NewInt(params.InitialBaseFee), new(big.Int), data, nil, true)}

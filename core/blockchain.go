...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	updateHead := func(head *types.Block) bool {
		...
		if bc.CurrentHeader().Number.Cmp(head.Number()) >= 0 {
			reorg, err := bc.forker.ReorgNeeded(bc.Engine(), bc.CurrentFastBlock().Header(), head.Header())
			if err != nil {
...
func (bc *BlockChain) writeBlockAndSetHead(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	reorg, err := bc.forker.ReorgNeeded(bc.Engine(), currentBlock.Header(), block.Header())
	if err != nil {
		return NonStatTy, err
	}
	if reorg {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals, setHead bool) (int, error) {
	...
	if bc.skipBlock(err, it) {
		...
		for block != nil && bc.skipBlock(err, it) {
			reorg, err = bc.forker.ReorgNeeded(bc.Engine(), current.Header(), block.Header())
			if err != nil {
				return it.index, err
			}
			if reorg {
	...
	for ; block != nil && err == nil || errors.Is(err, ErrKnownBlock); block, err = it.next() {
		...
		stats.report(chain, it.index, dirty, proctime, wtime)
	}
...
func (bc *BlockChain) insertSideChain(block *types.Block, it *insertIterator) (int, error) {
	...
	for ; block != nil && errors.Is(err, consensus.ErrPrunedAncestor); block, err = it.next() {
		...
		if number := block.NumberU64(); current.NumberU64() >= number {
			...
			if canonical != nil && canonical.Root() == block.Root() {
				...
				if params.DisableGhostStateAttackProtection {
					log.Warn("Ghost-State-Attack detected!!! Continuing sidechain import at your willing. Please turn off `disableghoststateattackprotection` option.", "number", block.NumberU64())
				} else {
	...
	reorg, err := bc.forker.ReorgNeeded(bc.Engine(), current.Header(), lastBlock.Header())
	if err != nil {
		return it.index, err
	}
	if !reorg {

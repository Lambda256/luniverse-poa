...
//func (bc *BlockChain) GetBlockState(hash common.Hash, number uint64) (*state.StateDB, error) {
...
func (bc *BlockChain) WriteBlockWithState(block *types.Block, receipts []*types.Receipt, state *state.StateDB) (status WriteStatus, err error) {
	...
	var reorgCtx []interface{}
	reorg := externTd.Cmp(localTd) > 0
	if reorg {
		reorgCtx = append(reorgCtx, "REORG by TD", localTd, externTd)
	}
	currentBlock = bc.CurrentBlock()
	if !reorg && externTd.Cmp(localTd) == 0 {
		...
		if newBlockNumber < currentBlockNumber {
			reorgCtx = append(reorgCtx, "REORG by block number", currentBlockNumber, newBlockNumber)
			reorg = true
		} else if newBlockNumber == currentBlockNumber {
			// Instead of random comparison (i.e., `mrand.Float64() < 0.5`),
			// we will use (1) block timestamp, (2) seal timestamp, and lastly (3) block hash.
			newBlockTime := block.Time().Int64()
			currentBlockTime := currentBlock.Time().Int64()
			if newBlockTime < currentBlockTime {
				reorgCtx = append(reorgCtx, "REORG by block time (sec)", currentBlockTime, newBlockTime)
				reorg = true
			} else if newBlockTime == currentBlockTime {
				newBlockSealTime := bc.engine.ExtractSealTimeUint64(block.Header())
				currentBlockSealTime := bc.engine.ExtractSealTimeUint64(currentBlock.Header())
				if newBlockSealTime < currentBlockSealTime {
					reorgCtx = append(reorgCtx, "REORG by seal time (ms)", currentBlockSealTime, newBlockSealTime)
					reorg = true
				} else if newBlockSealTime == currentBlockSealTime {
					newBlockHash := block.Hash()
					currentBlockHash := currentBlock.Hash()
					if bytes.Compare(newBlockHash[:], currentBlockHash[:]) < 0 {
						reorgCtx = append(reorgCtx, "REORG by block hash", currentBlockHash, newBlockHash)
						reorg = true
					}
				}
			}
	...
	if reorg {
		...
		if block.ParentHash() != currentBlock.Hash() {
			if len(reorgCtx) > 0 {
				log.Info((reorgCtx[0]).(string), "current", reorgCtx[1], "new", reorgCtx[2])
			}
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	log.Info("Executing reorg()", "oldBlock.Number", oldBlock.Number(), "oldBlock.Hash", oldBlock.Hash(), "newBlock.Number", newBlock.Number(), "newBlock.Hash", newBlock.Hash())
	var (

...
type Backend interface {
	...
	StateAtTransaction(ctx context.Context, block *types.Block, txIndex int, reexec uint64) (*common.ConsensusConfig, core.Message, vm.BlockContext, *state.StateDB, error)
}
...
func (api *API) traceChain(ctx context.Context, start, end *types.Block, config *TraceConfig) (*rpc.Subscription, error) {
		...
		go func() {
			...
			for task := range tasks {
				...
				for i, tx := range task.block.Transactions() {
					msg, _ := tx.AsMessage(api.backend.ChainConfig(), signer, task.block.BaseFee())
					txctx := &Context{
						BlockHash: task.block.Hash(),
						TxIndex:   i,
						TxHash:    tx.Hash(),
					}
					res, err := api.traceTx(task.consensusConfig, localctx, msg, txctx, blockCtx, task.statedb, config)
					if err != nil {
	...
	go func() {
		...
		for number = start.NumberU64(); number < end.NumberU64(); number++ {
			...
			if api.backend.ChainConfig() != nil && api.backend.ChainConfig().Clique != nil && api.backend.ChainConfig().Clique.HasConsensusConfiguration() {
				//if parentHeader == nil {
				//	// TODO: winnerxg - how to on genesis block?
				//	_parent, err := api.blockByNumberAndHash(localctx, rpc.BlockNumber(block.NumberU64()-1), block.ParentHash())
				//	if err != nil {
				//		failed = fmt.Errorf("parent block #%d not found", block.NumberU64()-1)
				//		break
				//	}
				//	parentHeader = _parent.Header()
				//}
				consensusConfig = core.RetrieveConsensusConfigurations(api.backend.BlockChain(), block.Header(), statedb, api.backend.ChainConfig())
			}
			...
			select {
			case tasks <- &blockTraceTask{consensusConfig: consensusConfig, statedb: statedb.Copy(), block: next, rootref: block.Root(), results: make([]*txTraceResult, len(txs))}:
			case <-notifier.Closed():
				return
			}
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	var consensusConfig *common.ConsensusConfig = nil
	if api.backend.ChainConfig() != nil && api.backend.ChainConfig().Clique != nil && api.backend.ChainConfig().Clique.HasConsensusConfiguration() {
		consensusConfig = core.RetrieveConsensusConfigurations(api.backend.BlockChain(), parent.Header(), statedb, api.backend.ChainConfig())
	}
	var (
		roots              []common.Hash
	...
	for i, tx := range block.Transactions() {
		var (
			msg, _    = tx.AsMessage(api.backend.ChainConfig(), signer, block.BaseFee())
			txContext = core.NewEVMTxContext(msg)
			vmenv     = vm.NewEVM(vmctx, txContext, statedb, chainConfig, vm.Config{})
		)
		statedb.Prepare(tx.Hash(), i)
		if _, err := core.ApplyMessage(consensusConfig, vmenv, msg, new(core.GasPool).AddGas(msg.Gas()+(params.TxGas*2))); err != nil {
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range jobs {
				msg, _ := txs[task.index].AsMessage(api.backend.ChainConfig(), signer, block.BaseFee())
				txctx := &Context{
					BlockHash: blockHash,
					TxIndex:   task.index,
					TxHash:    txs[task.index].Hash(),
				}
				res, err := api.traceTx(consensusConfig, ctx, msg, txctx, blockCtx, task.statedb, config)
				if err != nil {
...
func (api *API) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		statedb.Prepare(tx.Hash(), i)
		_, err = core.ApplyMessage(consensusConfig, vmenv, msg, new(core.GasPool).AddGas(msg.Gas()+(params.TxGas*2)))
		if writer != nil {
			writer.Flush()
		}
		if dump != nil {
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	consensusConfig, msg, vmctx, statedb, err := api.backend.StateAtTransaction(ctx, block, int(index), reexec)
	if err != nil {
		return nil, err
	}
	txctx := &Context{
	...
	return api.traceTx(consensusConfig, ctx, msg, txctx, vmctx, statedb, config)
}
...
func (api *API) TraceCall(ctx context.Context, args ethapi.TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, config *TraceCallConfig) (interface{}, error) {
	...
	// Execute the trace
	msg, err := args.ToMessage(api.backend.ChainConfig(), params.TxGasLimit /*api.backend.RPCGasCap()*/, nil /*block.BaseFee()*/)
	if err != nil {
		return nil, err
	}
	vmctx := core.NewEVMBlockContext(block.Header(), api.chainContext(ctx), nil)
	...
	return api.traceTx(consensusConfig, ctx, msg, new(Context), vmctx, statedb, traceConfig)
}

// traceTx configures a new tracer according to the provided configuration, and
// executes the given message in the provided environment. The return value will
// be tracer dependent.
func (api *API) traceTx(consensusConfig *common.ConsensusConfig, ctx context.Context, message core.Message, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	...
	result, err := core.ApplyMessage(consensusConfig, vmenv, message, new(core.GasPool).AddGas(message.Gas()+(params.TxGas*2)))
	if err != nil {

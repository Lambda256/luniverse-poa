...
func (env *blockExecutionEnv) commitTransaction(consensusConfig *common.ConsensusConfig, tx *types.Transaction, coinbase common.Address) error {
	vmconfig := *env.chain.GetVMConfig()
	snap := env.state.Snapshot()
	receipt, err := core.ApplyTransaction(consensusConfig, env.chain.Config(), env.chain, &coinbase, env.gasPool, env.state, env.header, tx, &env.header.GasUsed, vmconfig)
	if err != nil {
...
func (api *ConsensusAPI) assembleBlock(parentHash common.Hash, params *PayloadAttributesV1) (*ExecutableDataV1, error) {
	...
	consensusConfig, err := api.engine.GetConsensusConfigurations(header, nil)
	if err != nil {
		return nil, err
	}
	if err := api.engine.Prepare(consensusConfig, bc, header); err != nil {
		return nil, err
	}
	...
	for {
		...
		err = env.commitTransaction(consensusConfig, tx, coinbase)
		switch err {
	...
	// Create the block.
	block, err := api.engine.FinalizeAndAssemble(consensusConfig, bc, header, env.state, transactions, nil /* uncles */, env.receipts)
	if err != nil {
		return nil, err
	}

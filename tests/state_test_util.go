...
func (t *StateTest) RunNoVerify(subtest StateSubtest, vmconfig vm.Config, snapshotter bool) (*snapshot.Tree, *state.StateDB, common.Hash, error) {
	...
	post := t.json.Post[subtest.Fork][subtest.Index]
	msg, err := t.json.Tx.toMessage(config, post, baseFee)
	if err != nil {
	...
	if _, err := core.ApplyMessage(nil, evm, msg, gaspool); err != nil {
...
func (tx *stTransaction) toMessage(chainConfig *params.ChainConfig, ps stPostState, baseFee *big.Int) (core.Message, error) {
	...
	msg := types.NewMessage(chainConfig, from, to, tx.Nonce, value, gasLimit, gasPrice,

...
func (hc *HeaderChain) writeHeadersAndSetHead(headers []*types.Header, forker *ForkChoice) (*headerWriteResult, error) {
	...
	if reorg, err := forker.ReorgNeeded(hc.Engine(), hc.CurrentHeader(), lastHeader); err != nil {
		return nil, err
	} else if !reorg {
		if inserted != 0 {
...
func (bc *HeaderChain) GetBlockState(hash common.Hash, number uint64) (*state.StateDB, error) {
	return nil, nil
}

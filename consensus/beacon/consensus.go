...
func (beacon *Beacon) Prepare(consensusConfig *common.ConsensusConfig, chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if !reached {
		return beacon.ethone.Prepare(consensusConfig, chain, header)
	}
...
func (beacon *Beacon) Finalize(consensusConfig *common.ConsensusConfig, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header) {
	// Finalize is different with Prepare, it can be used in both block generation
	// and verification. So determine the consensus rules by header type.
	if !beacon.IsPoSHeader(header) {
		beacon.ethone.Finalize(consensusConfig, chain, header, state, txs, uncles)
		return
	}
...
func (beacon *Beacon) FinalizeAndAssemble(consensusConfig *common.ConsensusConfig, chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	...
	if !beacon.IsPoSHeader(header) {
		return beacon.ethone.FinalizeAndAssemble(consensusConfig, chain, header, state, txs, uncles, receipts)
	}
	// Finalize and assemble the block
	beacon.Finalize(consensusConfig, chain, header, state, txs, uncles)
...
func (beacon *Beacon) Seal(consensusConfig *common.ConsensusConfig, chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	if !beacon.IsPoSHeader(block.Header()) {
		return beacon.ethone.Seal(consensusConfig, chain, block, results, stop)
	}
...
func (beacon *Beacon) APIs(chain consensus.ChainHeaderReader, ethapis []rpc.API) []rpc.API {
	return beacon.ethone.APIs(chain, ethapis)
}
...
func (beacon *Beacon) GetConsensusConfigurations(header *types.Header, parents []*types.Header) (*common.ConsensusConfig, error) {
	return beacon.ethone.GetConsensusConfigurations(header, parents)
}

func (beacon *Beacon) SetBlockPeriod(blockPeriod uint64) {
	beacon.ethone.SetBlockPeriod(blockPeriod)
}

func (beacon *Beacon) SetTargetGasLimit(targetGasLimit *big.Int) {
	beacon.ethone.SetTargetGasLimit(targetGasLimit)
}

func (beacon *Beacon) SetTargetGasLimitCalculation(targetGasLimitCalculation bool) {
	beacon.ethone.SetTargetGasLimitCalculation(targetGasLimitCalculation)
}

func (beacon *Beacon) SetTxGasLimit(txGasLimit *big.Int) {
	beacon.ethone.SetTxGasLimit(txGasLimit)
}

func (beacon *Beacon) SetGasPrice(gasPrice *big.Int) {
	beacon.ethone.SetGasPrice(gasPrice)
}

func (beacon *Beacon) ExtractSealTimeUint64(header *types.Header) uint64 {
	return beacon.ethone.ExtractSealTimeUint64(header)
}

...
func (ethash *Ethash) Seal(consensusConfig *common.ConsensusConfig, chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if ethash.shared != nil {
		return ethash.shared.Seal(consensusConfig, chain, block, results, stop)
	}
	...
	go func() {
		...
		case <-ethash.update:
			...
			close(abort)
			if err := ethash.Seal(consensusConfig, chain, block, results, stop); err != nil {

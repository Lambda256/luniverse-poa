...
func New(conf *Config) (*Node, error) {
	...
	node := &Node{
		config:        conf,
		inprocHandler: rpc.NewServer(conf.IsRPCDebugLogging()),
	...
	node.http = newHTTPServer(node.log, conf.HTTPTimeouts, conf.IsRPCDebugLogging())
	node.ws = newHTTPServer(node.log, rpc.DefaultHTTPTimeouts, conf.IsRPCDebugLogging())
	node.ipc = newIPCServer(node.log, conf.IPCEndpoint(), conf.IsRPCDebugLogging())
...
func (n *Node) SetDebugLogging(enable bool) {
	n.inprocHandler.SetDebugLogging(enable)
	n.ipc.SetDebugLogging(enable)
	n.http.SetDebugLogging(enable)
	n.ws.SetDebugLogging(enable)
}
